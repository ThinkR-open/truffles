...
test_that("get_info_oak works", {
  ...
  expect_equal(
    get_info_oak(dboak, "2"),
    list(type = "Sessile oak", planting_date = as.Date("2018-03-15"))
  )

  expect_error(get_info_oak(iris, "2"))

  expect_error(get_info_oak(iris, mtcars), regexp = "theidoak must be a character")
})
